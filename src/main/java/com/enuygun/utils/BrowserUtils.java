...
import com.enuygun.enums.FailureHandling;
...
import com.enuygun.report.ExtentTestManager;
...
public class BrowserUtils {
    ...
    @Step("Drag from element {0} to X={1}, Y={2}")
    public static boolean dragAndDropToOffset(WebElement fromElement, int X, int Y) {
        smartWait();
    ...
    public static String getTextElement(WebElement element) {
        smartWait();
        AllureManager.saveTextLog("==> The Text is: " + waitForElementVisible(element).getText());
        return waitForElementVisible(element).getText().trim();
    }
...
//    }
    ...
    public static WebElement waitForElementVisible(WebElement element) {
        ...
        try {
            WebDriverWait wait = new WebDriverWait(DriverManager.getDriver(), Duration.ofSeconds(FrameworkConstants.WAIT_EXPLICIT - 10));
            boolean check = isElementVisible(element, 10);
            if (check) {
            ...
            LogUtils.error("❌ Timeout waiting for the element Visible. " + element.toString());
    ...
    public static WebElement waitForElementPresent(WebElement element) {
        try {
            ...
            LogUtils.error("❌ Element not exist. (waitForElementPresent) " + element.toString());
            Assert.fail("❌ Element not exist. (waitForElementPresent) " + element.toString());
        }
        return null;
    }
    ...
    public static void waitLoader(int time) {
        List<WebElement> loaderBoxes = DriverManager.getDriver()
                .findElements(By.xpath("//*[@data-testid=\"weg-loader\" or @data-test-id=\"weg-loader\"]"));
        if (!loaderBoxes.isEmpty()) {
            for (WebElement loaderBox : loaderBoxes) {
                if (isElementVisible(loaderBox, 0)) {
                    waitForElementToBeInvisible(loaderBox, time);
                }
            }
        }
    }
    ...
    public static void waitForElementToBeInvisible(WebElement element, int time) {
        try {
            ...
            LogUtils.error("Loader visible");
        }
    }

    public static void actionDragAndDropBy(WebElement element, int x, int y) {
        Actions actions = new Actions(DriverManager.getDriver());
    ...
    public static void scrollToElement(WebElement element) {
        try {
            ...
            LogUtils.info("Successfully scrolled to Object : ");
        } catch (Exception e) {
            LogUtils.error("Could not scroll to Element");
            Assert.fail("Could not scroll to Element");
        }
    }
